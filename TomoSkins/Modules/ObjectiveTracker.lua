...
TomoSkins_OT = {}
local OT = TomoSkins_OT

-- =====================================
-- LOCALS & CACHES
-- =====================================

local ADDON_FONT       = "Interface\\AddOns\\TomoSkins\\Assets\\Fonts\\Poppins-Medium.ttf"
local ADDON_FONT_BOLD  = "Interface\\AddOns\\TomoSkins\\Assets\\Fonts\\Poppins-SemiBold.ttf"
local ADDON_FONT_BLACK = "Interface\\AddOns\\TomoSkins\\Assets\\Fonts\\Poppins-Bold.ttf"
local ADDON_TEXTURE    = "Interface\\AddOns\\TomoSkins\\Assets\\Textures\\tomoaniki"
-- locale inlined
...
local function S()
    return TomoSkinsDB and TomoSkinsDB.objectiveTracker or {}
end
...
local function CreateOrUpdateBackground()
    ...
    if not skinFrame then
        skinFrame = CreateFrame("Frame", "TomoSkins_OTSkin", UIParent)
...
local function CreateOrUpdateHeader()
    ...
    if not headerBar then
        headerBar = CreateFrame("Frame", "TomoSkins_OTHeader", skinFrame)
        ...
        headerOptions:SetScript("OnClick", function()
            if TomoSkins_OpenConfig then TomoSkins_OpenConfig() end
        end)
...
function OT.SetEnabled(value)
    if not TomoSkinsDB or not TomoSkinsDB.objectiveTracker then return end
    TomoSkinsDB.objectiveTracker.enabled = value
    if value then
        OT.Enable()
        isInitialized = true
    else
