...
TomoSkins_IP = {}
local IP = TomoSkins_IP

-- =====================================
-- LOCALS
-- =====================================

local FONT       = "Interface\\AddOns\\TomoSkins\\Assets\\Fonts\\Poppins-Medium.ttf"
local FONT_BOLD  = "Interface\\AddOns\\TomoSkins\\Assets\\Fonts\\Poppins-SemiBold.ttf"
local FONT_BLACK = "Interface\\AddOns\\TomoSkins\\Assets\\Fonts\\Poppins-Bold.ttf"
-- locale inlined
...
local function DB()
    return TomoSkinsDB and TomoSkinsDB.infoPanel
end
...
local function CreateUI()
    ...
    if TomoSkins_Utils and TomoSkins_Utils.GetClassColor then
        classR, classG, classB = TomoSkins_Utils.GetClassColor()
    end

    -- =========================================
    -- ZONE BAR (above minimap)
    -- =========================================
    zoneBar = CreateFrame("Frame", "TomoSkins_ZoneBar", minimap, "BackdropTemplate")
    ...
    -- =========================================
    clockBar = CreateFrame("Button", "TomoSkins_ClockBar", minimap, "BackdropTemplate")
    ...
    clockBar:SetScript("OnClick", function(self, button)
        ...
        elseif button == "RightButton" then
            if IsShiftKeyDown() then
                ...
                print("|cfff43f5eTomoSkins|r " .. string.format(L["time_format_msg"], fmt))
            else
                db.useServerTime = not db.useServerTime
                local mode = db.useServerTime and L["time_server"] or L["time_local"]
                print("|cfff43f5eTomoSkins|r " .. string.format(L["time_mode_msg"], mode))
            end
            IP.Update()
        end
    end)
...
function IP.UpdateAppearance()
    ...
    local useClass = TomoSkinsDB.minimap.borderColor == "class"
