...
local TEXTURE = "Interface\\AddOns\\TomoFrames\\Assets\\Textures\\tomoaniki"
local FONT = "Interface\\AddOns\\TomoFrames\\Assets\\Fonts\\Poppins-Medium.ttf"
...
function UF_Elements.CreateCastbar(parent, unit, settings)
    ...
    local tex = (TomoFramesDB and TomoFramesDB.texture) or TEXTURE
    local font = (TomoFramesDB and TomoFramesDB.font) or FONT

    local castbar = CreateFrame("StatusBar", "TomoFrames_Castbar_" .. unit, parent)
    ...
    local cbColors = TomoFramesDB and TomoFramesDB.castbarColor
    ...
    if unit == "player" then
        ...
        TomoFrames_Utils.SetupDraggable(castbar, function()
    ...
    local niColors = TomoFramesDB and TomoFramesDB.castbarNIColor
    ...
    local function CheckCast(self, isInterrupt)
        ...
        if isInterrupt then
            self.niOverlay:SetAlpha(0)
            ResetState(self)
            local intCol = TomoFramesDB and TomoFramesDB.castbarInterruptColor
            if intCol then
