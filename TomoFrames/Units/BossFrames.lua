...
TomoFrames_BF = TomoFrames_BF or {}
local BF = TomoFrames_BF
local E = UF_Elements
...
local function CreateBossFrame(bossIndex)
    ...
    local db = TomoFramesDB.bossFrames
    if not db then return nil end

    local tex = TomoFramesDB.texture or "Interface\\AddOns\\TomoFrames\\Assets\\Textures\\tomoaniki"
    local font = TomoFramesDB.fontFamily or TomoFramesDB.font or "Interface\\AddOns\\TomoFrames\\Assets\\Fonts\\Poppins-Medium.ttf"
    local fontSize = TomoFramesDB.fontSize or 12

    local width = db.width or 200
    local height = db.height or 28

    -- Main frame (SecureUnitButtonTemplate for click-targeting)
    local frame = CreateFrame("Button", "TomoFrames_Boss_" .. bossIndex, UIParent, "SecureUnitButtonTemplate")
...
function BF.Initialize()
    ...
    print("|cfff59e0bTomoFrames|r " .. "Boss frames initialisées.")
end
