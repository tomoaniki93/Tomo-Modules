...
TomoFrames_UF = TomoFrames_UF or {}
local UF = TomoFrames_UF
local E = UF_Elements
...
local function CreateUnitFrame(unit, settings)
    local frame = CreateFrame("Button", "TomoFrames_UF_" .. unit, UIParent, "SecureUnitButtonTemplate")
    ...
    -- Draggable
    TomoFrames_Utils.SetupDraggable(frame, function()
...
local function UpdateHealth(frame)
    ...
    local settings = TomoFramesDB[unit]
...
local function UpdateName(frame)
    ...
    local settings = TomoFramesDB[frame.unit]
    ...
    if UnitIsPlayer(frame.unit) then
        local r, g, b = TomoFrames_Utils.GetClassColor(frame.unit)
    ...
    if settings.nameTruncate and settings.nameTruncateLength then
        ...
        local dbFontSize = (TomoFramesDB and TomoFramesDB.fontSize) or 12
...
function UF.ToggleLock()
    ...
    for unit, frame in pairs(frames) do
        ...
        if not isLocked then
            ...
            local unitSettings = TomoFramesDB[unit]
    ...
    if TomoFrames_BF and TomoFrames_BF.ToggleLock then
        TomoFrames_BF.ToggleLock()
    end
end
...
function UF.RefreshUnit(unitKey)
    ...
    local settings = TomoFramesDB[unitKey]
    if not frame or not settings then return end

    local globalDB = TomoFramesDB
    local font = globalDB.fontFamily or globalDB.font or "Interface\\AddOns\\TomoFrames\\Assets\\Fonts\\Poppins-Medium.ttf"
...
function UF.Initialize()
    if not TomoFramesDB then return end
    if not TomoFramesDB.enabled then return end
    ...
    -- Hide Blizzard
    if TomoFramesDB.hideBlizzardFrames then
        HideBlizzardFrames()
    end
    ...
    print("|cfff59e0bTomoFrames|r " .. "Initialisé.")
end
...
-- standalone, no module registration
