...
TomoCooldown_RB = TomoCooldown_RB or {}
local RB = TomoCooldown_RB

local TEXTURE = "Interface\\AddOns\\TomoCooldown\\Assets\\Textures\\tomoaniki"
local FONT = "Interface\\AddOns\\TomoCooldown\\Assets\\Fonts\\Poppins-Medium.ttf"
...
local function GetSettings()
    return TomoCooldownDB and TomoCooldownDB.resourceBars
end
...
local function CreatePrimaryBar(parent, width, height)
    local tex = (TomoCooldownDB.resourceBars and TomoCooldownDB.resourceBars.texture) or TEXTURE

    local bar = CreateFrame("StatusBar", "TomoCooldown_RB_Primary", parent)
...
local function CreatePointDisplay(parent, maxPoints, width, height, colorKey)
    local frame = CreateFrame("Frame", "TomoCooldown_RB_Points", parent)
...
local function CreateRuneDisplay(parent, width, height)
    local frame = CreateFrame("Frame", "TomoCooldown_RB_Runes", parent)
    frame:SetSize(width, height)
...
local function CreateStaggerBar(parent, width, height)
    local tex = (TomoCooldownDB.resourceBars and TomoCooldownDB.resourceBars.texture) or TEXTURE
    local bar = CreateFrame("StatusBar", "TomoCooldown_RB_Stagger", parent)
    bar:SetSize(width, height)
    bar:SetStatusBarTexture(tex)
    bar:GetStatusBarTexture():SetHorizTile(false)
...
local function CreateDruidManaBar(parent, width, height)
    local tex = (TomoCooldownDB.resourceBars and TomoCooldownDB.resourceBars.texture) or TEXTURE
    local bar = CreateFrame("StatusBar", "TomoCooldown_RB_DruidMana", parent)
    bar:SetSize(width, height)
    bar:SetStatusBarTexture(tex)
    bar:GetStatusBarTexture():SetHorizTile(false)
...
local function BuildResourceDisplay()
    ...
    if not container then
        container = CreateFrame("Frame", "TomoCooldown_RB_Container", UIParent)
        container:SetClampedToScreen(true)
        TomoCooldown_Utils.SetupDraggable(container, function()
...
local function SyncWithEssentialCooldowns()
    ...
    if ecv then
        ...
        if w and w > 0 then
            s.width = w
            BuildResourceDisplay()
            print("|cff06b6d4TomoCooldown|r " .. string.format("Largeur synchronisée : %d px", math.floor(w)))
        end
    end
end
...
function RB.Initialize()
    ...
    if not TomoCooldownDB then return end
    local s = GetSettings()
    if not s or not s.enabled then return end
...
function RB.ToggleLock()
    ...
    if container.SetLocked then
        ...
        if not locked then
            print("|cff06b6d4TomoCooldown|r " .. "Barres verrouillées.")
        else
            print("|cff06b6d4TomoCooldown|r " .. "Barres déverrouillées — glissez pour repositionner.")
        end
    end
end
